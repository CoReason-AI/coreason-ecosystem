...
class SkyPilotActuator:
    ...
    async def provision_node(self, target: SkyPilotTarget) -> dict[str, Any]:
        ...
        if target.hardware_profile:
             # Example mapping: HardwareProfile(accelerator_type="A100") -> "A100:1"
        ...
        if (
            ...
            setup_cmds.append(f"echo {payload_b64} | base64 -d > /etc/coreason/payload.json")
            setup_cmds.append("/opt/coreason/bin/bootstrap.sh")

        task = sky.Task(
            setup="\n".join(setup_cmds) if setup_cmds else 'echo "SkyPilot Node Booting..."',
            run='sleep infinity', # Keep the node alive for the mesh
        )
        task.set_resources(resources)

        logger.info(f"SkyPilot: Launching cluster {cluster_name} with {accelerators} (spot={target.use_spot})...")
        ...
        for cluster in clusters:
            ...
            if name.startswith("coreason-sky-"):
                ...
                active_nodes.append({
                    ...
                    "status": status,
                    "provider": cloud,
                    "vram_capacity": 80.0 if "A100" in (str(cluster.get("resources", ""))) else 0.0 # Heuristic
                })
        return active_nodes
    ...
    async def execute_thermodynamic_guillotine(self, threshold_breached: bool) -> None:
        ...
        logger.critical("[SkyPilotActuator] Thermodynamic Guillotine: Severing all infrastructure!")
