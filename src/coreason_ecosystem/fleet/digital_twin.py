...
class CyberPhysicalDigitalTwin:
    ...
    async def dispatch_actuation_command(
        self, target_cid: str, command: Dict[str, Any]
    ) -> None:
        """
        ...
        if self.enforce_no_side_effects:
            logger.warning(
                f"ACTUATION BLOCKED: Digital Twin enforce_no_side_effects is active. Command {command} safely guillotined."
            )
            raise PermissionError(
                "Volumetric Guillotine: Real-world IoT actuation is disabled in this physics boundary."
            )
