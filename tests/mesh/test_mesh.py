...
def test_kademlia_dht_mock() -> None:
    ...
    missing_cid = "sha256:1111111111111111111111111111111111111111111111111111111111111111"
...
def test_zero_copy_streaming_mock() -> None:
    ...
    intent = CIDFetchIntent(
        target_cid=valid_cid
    )
    
    data = stream.handle_fetch_intent(intent)
    assert data == b"mocked_binary_content"
    
    intent_missing = CIDFetchIntent(
        target_cid=missing_cid
    )
