...
def test_init_command() -> None:
    with (
        patch(
            "coreason_ecosystem.utils.telemetry.start_otlp_background_worker"
        ) as mock_start,
        patch(
        ...
        patch(
            "coreason_ecosystem.cli.execute_init", new_callable=AsyncMock
        ) as mock_init,
    ):
...
def test_build_command() -> None:
    with (
        ...
        patch(
            "coreason_ecosystem.cli.execute_build", new_callable=AsyncMock
        ) as mock_build,
    ):
...
def test_doctor_command() -> None:
    with (
        ...
        patch(
            "coreason_ecosystem.cli.execute_oracle_diagnostic", new_callable=AsyncMock
        ) as mock_doc,
    ):
...
def test_sync_command() -> None:
    with (
        ...
        patch(
            "coreason_ecosystem.cli.execute_sync", new_callable=AsyncMock
        ) as mock_sync,
    ):
