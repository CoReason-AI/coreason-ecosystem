...
async def test_compile_and_hash_rs_fallback(tmp_path: Path) -> None:
    ...
    with patch(
        "coreason_ecosystem.orchestration.build.asyncio.create_subprocess_exec",
        ...
        target_wasm = (
            cargo_dir / "target" / "wasm32-wasip1" / "release" / "other_name.wasm"
        )
...
async def test_execute_build_success(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "coreason_ecosystem.orchestration.build.compile_and_hash",
            new_callable=AsyncMock,
        ) as mock_compile,
    ):
