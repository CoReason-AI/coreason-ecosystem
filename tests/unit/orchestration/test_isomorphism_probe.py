...
@pytest.mark.asyncio
async def test_execute_oracle_diagnostic_all_success(
...
@pytest.mark.asyncio
async def test_execute_oracle_diagnostic_all_failures(
    tmp_path: Path, capsys, monkeypatch
):
    monkeypatch.chdir(tmp_path)
...
@pytest.mark.asyncio
async def test_execute_oracle_diagnostic_http_errors(
    tmp_path: Path, capsys, monkeypatch
):
    schema_path = tmp_path / "coreason_ontology.schema.json"
    schema_path.write_bytes(b"{}")
...
@pytest.mark.asyncio
async def test_execute_oracle_diagnostic_epistemic_http_error(
    tmp_path: Path, capsys, monkeypatch
):
    schema_path = tmp_path / "coreason_ontology.schema.json"
    schema_path.write_bytes(b"{}")
...
@pytest.mark.asyncio
async def test_execute_oracle_diagnostic_epistemic_timeout(
    tmp_path: Path, capsys, monkeypatch
):
    schema_path = tmp_path / "coreason_ontology.schema.json"
    schema_path.write_bytes(b"{}")
