...
async def test_wait_for_postgres_timeout() -> None:
    with (
        ...
        with pytest.raises(
            TimeoutError,
            match="PostgreSQL failed to achieve application-layer readiness.",
        ):
...
async def test_wait_for_port_timeout() -> None:
    with (
        ...
        with pytest.raises(
            TimeoutError, match="Fallback check failed. Port 8000 never bound."
        ):
            await wait_for_port(8000, timeout=1.0)


@pytest.fixture
def mock_up_dependencies() -> Generator[
    tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock], None, None
]:
    with (
        ...
        patch(
            "coreason_ecosystem.orchestration.up.wait_for_port", new_callable=AsyncMock
        ) as mock_port,
        patch(
...
async def test_execute_up_success(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
    ...
    )  # teardown, postgres, temporal, runtime, observability
...
@pytest.mark.asyncio
async def test_execute_up_teardown_error(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
...
@pytest.mark.asyncio
async def test_execute_up_postgres_error(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
    mock_exec, _, _, _, _ = mock_up_dependencies
    proc_ok = AsyncMock()
...
@pytest.mark.asyncio
async def test_execute_up_postgres_timeout(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
...
@pytest.mark.asyncio
async def test_execute_up_temporal_timeout(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
...
@pytest.mark.asyncio
async def test_execute_up_daemon_timeout(
    mock_up_dependencies: tuple[AsyncMock, AsyncMock, AsyncMock, AsyncMock, MagicMock],
) -> None:
...
async def test_provision_swarm_topology() -> None:
    ...
    with patch(
        "coreason_ecosystem.orchestration.up.execute_up", new_callable=AsyncMock
    ) as mock_up:
