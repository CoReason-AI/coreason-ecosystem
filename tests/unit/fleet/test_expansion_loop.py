...
async def test_von_neumann_expansion_daemon_economic_guillotine() -> None:
    ...
    with (
        patch("coreason_ecosystem.fleet.expansion_loop.coreason_aggregate_vram_demand_gb._value.get", return_value=40.0),
        patch("coreason_ecosystem.fleet.expansion_loop.PulumiActuator") as mock_actuator_cls,
        patch("coreason_ecosystem.fleet.expansion_loop.assess_thermodynamic_expenditure", new_callable=AsyncMock) as mock_assess
    ):
...
async def test_von_neumann_expansion_daemon_provision_success() -> None:
    ...
    with (
        patch("coreason_ecosystem.fleet.expansion_loop.coreason_aggregate_vram_demand_gb._value.get", return_value=10.0),
        patch("coreason_ecosystem.fleet.expansion_loop.PulumiActuator") as mock_actuator_cls,
        patch("coreason_ecosystem.fleet.expansion_loop.assess_thermodynamic_expenditure", new_callable=AsyncMock) as mock_assess,
        ...
        mock_actuator.reconcile_state = AsyncMock(return_value=[{"vram_capacity": 5, "market_type": "spot"}])
...
async def test_von_neumann_expansion_daemon_provision_spot() -> None:
    ...
    with (
        ...
        mock_actuator.reconcile_state = AsyncMock(return_value=[{"vram_capacity": 5, "market_type": "on-demand"}])
        mock_actuator.provision_node = AsyncMock()
        mock_actuator_cls.return_value = mock_actuator
...
async def test_von_neumann_expansion_daemon_exception_handling() -> None:
    ...
    with (
        patch("coreason_ecosystem.fleet.expansion_loop.coreason_aggregate_vram_demand_gb._value.get", side_effect=RuntimeError("Test error")),
