...
def test_generate_dynamic_docs_success(tmp_path: Path):
...
def test_generate_dynamic_docs_defaults_missing_schema(tmp_path: Path):
...
def test_generate_dynamic_docs_defaults_with_schema(tmp_path: Path):
    with patch("coreason_ecosystem.docs_generator.Path.cwd", return_value=tmp_path):
        schema_path = tmp_path / "coreason_ontology.schema.json"
