...
def test_deployment_continuum_helm_values() -> None:
    """
    ...
    assert prod_values["runtime"]["autoscaling"]["enabled"] is True, (
        "HPA must be enabled in production."
    )
    assert prod_values["runtime"]["autoscaling"]["maxReplicas"] >= 10, (
        "Production must scale to at least 10 replicas."
    )

    # Verify Local/Edge values
    assert local_values["global"]["environment"] == "local", (
        "Local environment must be 'local'"
    )
    assert local_values["runtime"]["autoscaling"]["enabled"] is False, (
        "HPA must be disabled on the edge/laptop."
    )
    assert local_values["runtime"]["replicaCount"] == 1, (
        "Edge deployment must constrain replicas to 1."
    )


def test_edge_compose_integrity() -> None:
    """
    ...
    assert "otel-collector" in services, (
        "OTEL Collector sidecar is missing from Edge deployment"
    )
    assert "redpanda-connect" in services, (
        "Redpanda Connect is missing from Edge deployment"
    )

    gateway_env = services["coreason-master-gateway"].get("environment", [])
    has_otlp_endpoint = any(
        "COREASON_OTLP_ENDPOINT" in env_var for env_var in gateway_env
    )
    assert has_otlp_endpoint, (
        "Master Gateway must have COREASON_OTLP_ENDPOINT pointing to sidecar"
    )
