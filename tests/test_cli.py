...
def cli_state(monkeypatch):
    ...
    async def fake_up(): state.up_called = True
    async def fake_doctor(): state.doctor_called = True
    async def fake_sync(): state.sync_called = True
    def fake_docs(*args, **kwargs): state.docs_called = True
