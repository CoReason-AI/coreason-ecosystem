...
@patch("coreason_ecosystem.fleet.expansion_loop.PulumiActuator")
@patch("asyncio.sleep")
async def test_von_neumann_no_bids(
    mock_sleep: Any,
...
async def test_von_neumann_provision_bid(
    ...
    bid = MagicMock(
        provider="aws", bid_price_gwei=10, target_region="us-east-1", market_type="spot"
    )
...
@patch("coreason_ecosystem.fleet.expansion_loop.PulumiActuator")
@patch("asyncio.sleep")
async def test_von_neumann_provision_bid_on_demand(
    mock_sleep: Any,
    mock_actuator_cls: Any,
    mock_assess: Any,
    mock_registry: Any,
