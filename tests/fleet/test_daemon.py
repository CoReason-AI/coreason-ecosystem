...
def fleet_manager() -> Any:
    with (
        ...
        manager.driver.provision_node = AsyncMock(
            return_value={"stack_name": "test_stack"}
        )
...
async def test_autonomic_fleet_manager_scale_zero(
    ...
    fleet_manager.driver.reconcile_state.return_value = [
        {"stack_name": "stack1", "provider": "aws"}
    ]
    ...
    fleet_manager.driver.destroy_node.assert_called_once_with("stack1", "aws")
...
@pytest.mark.asyncio
@patch("asyncio.sleep", new_callable=AsyncMock)
async def test_autonomic_fleet_manager_scale_up_no_bid(
    mock_sleep: Any, fleet_manager: Any
) -> None:
    fleet_manager.driver.reconcile_state.return_value = [{"vram_capacity": -10.0}]
    ...
    fleet_manager.driver.provision_node.assert_not_called()
