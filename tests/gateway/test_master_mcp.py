...
async def test_federated_discovery(client: TestClient) -> None:
    ...
    mock_response.json.return_value = [{"name": "mock_extract", "description": "Mock clinical task", "inputSchema": {}}]
    
    with unittest.mock.patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_get.return_value = mock_response

        payload = {"domain_filter": ["clinical_extractor"]}
...
async def test_execution_and_receipt(client: TestClient) -> None:
    ...
    with unittest.mock.patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
...
async def test_execution_http_status_error(client: TestClient) -> None:
    ...
    def raise_http_status_error(*args, **kwargs) -> None:
        ...
        raise httpx.HTTPStatusError("Server Error", request=unittest.mock.Mock(), response=response)
